...
def index():
    if request.method == 'POST':
        content_pickle = session.get('data')
        if content_pickle:
        ...
        session['data'] = jsonpickle.encode(chat_session.history)
        print("pushing",jsonpickle.decode(session.get('data')))
        y = (response.text)
